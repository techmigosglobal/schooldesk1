...
void main() {
  test('payment-proof decision guard accepts legacy pending aliases only', () {
    ...
    final guard = source.substring(guardStart - 260, guardStart + 120);
    for (final status in [
      '"pending"',
      '"submitted"',
      ...
      expect(guard, contains(status));
    }
    expect(guard, isNot(contains('"initiated"')));
    expect(guard, isNot(contains('"clarification_required"')));
  });
