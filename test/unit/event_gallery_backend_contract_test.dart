...
void main() {
  test(
    'gallery JSON filters and approval notifications use the deployed API',
    () {
      final handler = File(
      ...
      expect(
        handler,
        contains('notifyUsersByRole(svc, school, "parent", galleryPayload)'),
      );
      expect(handler, contains('notifyUsersByRole(svc, school, "teacher", {'));
      expect(
        handler,
        contains(r'excludeUserId: `${existing.created_by ?? ""}`'),
      );
