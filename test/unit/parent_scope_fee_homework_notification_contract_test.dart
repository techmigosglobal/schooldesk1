...
void main() {
  test(
    'parent context, fee decisions, and homework notices retain child scope',
    () {
      final selector = File(
      ...
      expect(selector, contains('parentChildClassAndSectionLabel'));
      expect(selector, contains("_withPrefix(resolvedGrade, 'Class')"));
      expect(selector, contains("_withPrefix(section, 'Section')"));
      ...
      expect(
        resolver,
        contains("'student_id': data['student_id'].toString().trim()"),
      );
    },
  );
}
