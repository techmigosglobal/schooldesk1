...
void main() {
  test(
    'teacher leave uses routed form with real backend staff and leave ids',
    () {
      final screen = File(
        'lib/presentation/teacher_leave_screen/teacher_leave_screen.dart',
      ).readAsStringSync();
      final form = File(
        'lib/presentation/teacher_leave_screen/teacher_leave_request_form_screen.dart',
      ).readAsStringSync();
      final api = File(
      ...
      expect(
        api,
        contains('Future<List<Map<String, dynamic>>> getLeaveTypes()'),
      );
      expect(
        api,
      ...
      expect(
        guard,
        contains('AppRoutes.teacherLeaveRequestForm: {\'teacher\'}'),
      );
      expect(registry, contains('/teacher-leave-screen/request'));
      expect(main, contains('leaveCompat.GET("/balances"'));
    },
  );
}
