...
| [x] | AUTO-013 | Local ops readiness | `curl /ready`, `curl /metrics`, CI workflow inspection | Readiness/metrics endpoints are local-only verified and CI runs local Docker gates | `/ready` returned database/redis ok; `/metrics` returned backend/database/redis up; `.github/workflows/local-docker-ci.yml` added. |
...
| [!] | COORD-010 | Transport/library backend -> mobile UI | Backend transport/library routes and models exist; Flutter screens/routes are not first-class yet | Transport/library are documented release gaps until UI and local Docker positive lifecycle tests are added. |
...
| [!] | ADV-015.1 | Transport | Assign bus route and parent views route/driver | Backend has `/transport/*` CRUD and `GET /students/:id/transport` with linked-student access. Transport is backend-present but Flutter UI is still a release gap. | Not client-ready until Admin/Principal transport UI, Parent transport view, and Docker positive lifecycle tests are implemented. |
| [!] | ADV-016.1 | Library | Issue/return book and calculate fine | Backend has `/library/categories`, `/library/books`, `/library/issues` and `BookIssue.fine_per_day/fine_amount`. Library is backend-present but Flutter UI is still a release gap. | Not client-ready until catalog/issue/return UI and backend fine-calculation tests are added. |
