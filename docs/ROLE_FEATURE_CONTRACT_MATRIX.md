...
| Coordinator | Same school operations and user-access workflows as Principal, scoped to one assigned branch; non-financial reports | No fees, fee reports, payment approvals, or branch switching; cannot assign Principal, Admin, or Super Admin roles | API derives the assigned branch from the active user profile; finance routes, reports, dashboard fields, approval rows, and exports remain finance-leader scoped; account mutations remain school-scoped |
