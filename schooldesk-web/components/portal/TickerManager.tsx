...
export function TickerManager({ onNotify, compact = false }: { onNotify: (message: string) => void; compact?: boolean }) {
  ...
  return <section className={compact ? "surface ops-form-surface" : "ops-module gallery-workspace"}>
    <div className="ops-module-heading">
      <div><div className="ops-module-icon green" style={{ background: "#eef8f1", color: "#1d632f" }}><BellRing size={20} /></div><div><p className="ops-kicker">Public announcement</p><h2>Breaking News Bar</h2><p>Publish one short, scrolling announcement across the public website. Turn it off any time.</p></div></div>
    ...
    <form key={`${stringValue(ticker.breaking_news_text)}-${ticker.breaking_news_enabled === true}`} action={save} className="ops-detail-form" style={{ maxWidth: "760px" }}>
      <label className="field"><span>Announcement text <small>(240 characters maximum)</small></span><textarea name="breaking_news_text" maxLength={240} rows={3} defaultValue={stringValue(ticker.breaking_news_text)} placeholder="e.g. Admissions open for the 2026–27 academic year." disabled={loading} /></label>
      <label className="field checkbox-field"><input name="breaking_news_enabled" type="checkbox" defaultChecked={ticker.breaking_news_enabled === true} disabled={loading} /> <span>Show this announcement on the public website</span></label>
      <button className="primary-button" disabled={loading || saving}>{saving ? "Publishing…" : "Publish announcement"}</button>
    </form>
  </section>;
}
