...
import { buildFeeReceiptHtml } from "./fee-receipt";
...
function printReceipt(r: Row, stName: string, school: Row, invoice?: Row) {
...
export function FeesWorkspace({
  ...
  const [schoolProfile, setSchoolProfile] = useState<Row>({});
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      setSchoolProfile((currentSchool as Row) || {});
      setNotice("");
    } catch (event) {
              ...
              {view === "collections" && (
                <FinanceTable
                  ...
                  renderRow={(r) => {
                    ...
                    const amountPaid = Number(r.amount_paid ?? r.amount ?? 0);
                    const payDate = stringValue(r.paid_at ?? r.created_at).slice(0, 10);
                    const payMode = stringValue(r.payment_method ?? r.payment_mode);
                    const txnRef = stringValue(r.reference_number ?? receipt.transaction_ref ?? "");
                    const invoice = state.invoices.find(
                      (item) => stringValue(item.id) === stringValue(r.invoice_id),
                    );
                    return (
                      <tr key={stringValue(r.id)}>
                        ...
                        <td>
                          <button
                            ...
                            onClick={() => printReceipt(r, stName, schoolProfile, invoice)}
