...
import { CheckCircle2, CircleAlert, Eye, EyeOff, Globe2, Images, Pencil, Plus, RefreshCw, Trash2 } from "@/lib/lucide-react";
...
type MediaFilter = "all" | "image" | "video" | "visible" | "hidden";
...
export function WebsiteManager({ onNotify }: { onNotify: (message: string) => void }) {
  ...
  const visibleGallery = useMemo(() => {
    ...
    return gallery.filter((item) => {
      ...
      const matchesFilter = filter === "all" || (filter === "image" && !video) || (filter === "video" && video) || (filter === "visible" && published) || (filter === "hidden" && !published);
  ...
  const mediaCounts = useMemo(() => {
    const image = gallery.filter((item) => !isVideo(item)).length;
    const visible = gallery.filter((item) => item.is_published !== false).length;
    return { all: gallery.length, image, video: gallery.length - image, visible, hidden: gallery.length - visible };
  }, [gallery]);

  const mediaFilterOptions: Array<{ key: MediaFilter; label: string; count: number }> = [
    { key: "all", label: "All media", count: mediaCounts.all },
    { key: "image", label: "Images", count: mediaCounts.image },
    { key: "video", label: "Videos", count: mediaCounts.video },
    { key: "visible", label: "Displayed on website", count: mediaCounts.visible },
    { key: "hidden", label: "Hidden from website", count: mediaCounts.hidden },
  ];
  ...
  async function toggle(item: Row) {
    ...
    const eventPost = stringValue(item.source) === "event_post";
    const nextVisible = item.is_published === false;
    setOperation(`toggle:${itemId}`);
    setNotice("");
    // Update the card immediately so the control feels responsive while the
    // server persists the selection. The previous value is restored on error.
    setGallery((current) => current.map((row) => stringValue(row.id) === itemId ? { ...row, is_published: nextVisible } : row));
    try {
      if (eventPost) {
        await api(`event-posts/${stringValue(item.event_post_id)}`, {
          method: "PUT",
          body: JSON.stringify({ public_gallery_visible: nextVisible }),
        });
      } else {
        await api(`website/gallery/${itemId}`, {
          method: "PUT",
          body: JSON.stringify({
            title: item.title,
      ...
      onNotify(nextVisible ? "Media displayed on the public website." : "Media hidden from the public website.");
      void load(true);
    } catch (event) {
      setGallery((current) => current.map((row) => stringValue(row.id) === itemId ? { ...row, is_published: item.is_published } : row));
      setNotice(event instanceof Error ? event.message : "Unable to update media status");
    }
  ...
  return <section className="ops-module gallery-workspace">
    ...
    {loading && tab !== "ticker" ? <PortalModuleSkeleton variant="split" label="Loading website and media settings" /> : tab === "gallery" ? <div className="media-library-layout">
      <section className="surface media-upload-panel"><h3>Upload approved media</h3><p>Images: JPG, PNG, WebP up to 10 MB. Videos: MP4, WebM, MOV up to 50 MB.</p><form action={upload}><label className="field">Image or video<input name="file" type="file" accept="image/jpeg,image/png,image/webp,video/mp4,video/webm,video/quicktime" onChange={handleFileSelect} required /></label>{previewFileUrl && <div className="media-file-preview">{previewFileType.startsWith("video/") ? <video src={previewFileUrl} controls preload="metadata" /> : <img src={previewFileUrl} alt="Selected upload preview" />}</div>}<label className="field">Title<input name="title" placeholder="e.g. Nature exploration" /></label><label className="field">Accessible description<input name="alt_text" required placeholder="Describe the children and activity shown" /></label><label className="field">Caption <small>(optional)</small><input name="caption" placeholder="Short public caption" /></label><label className="field">Display order <small>(lower appears first)</small><input name="sort_order" type="number" min="0" defaultValue="0" /></label><label className="check-field"><input name="is_published" type="checkbox" value="true" defaultChecked />Display on the public website</label><button className="primary-button" disabled={uploading} aria-busy={uploading} type="submit">{uploading ? <LoadingIndicator label="Uploading…" compact announce={false} /> : <><Plus size={16} />Upload to gallery</>}</button></form></section>
      <section className="surface media-library-panel">
        <div className="media-visibility-summary" aria-label="Website gallery visibility summary">
          <article className="visible"><Eye size={20} /><div><span>Displayed on website</span><b>{mediaCounts.visible}</b></div></article>
          <article className="hidden"><EyeOff size={20} /><div><span>Hidden from website</span><b>{mediaCounts.hidden}</b></div></article>
          <article className="context"><Globe2 size={20} /><div><span>App School Gallery</span><p>These controls do not change in-app visibility.</p></div></article>
        </div>
        <div className="media-library-toolbar"><div><h3>Public media library</h3><small>{visibleGallery.length} item{visibleGallery.length === 1 ? "" : "s"} shown · Approved event posts from every app destination are listed here. Select a filter to see exactly what visitors can or cannot see.</small></div><label className="media-search"><span className="sr-only">Search media</span><input value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Search media" /></label></div>
        <div className="media-filter-row" aria-label="Filter media">
          {mediaFilterOptions.map(({ key, label, count }) => <button key={key} type="button" className={filter === key ? "active" : ""} aria-pressed={filter === key} onClick={() => setFilter(key)}><span>{label}</span><span className="media-filter-count">{count}</span></button>)}
        </div>
        {visibleGallery.length ? <div className="media-library-grid">{visibleGallery.map((item) => {
          const video = isVideo(item);
          const eventPost = stringValue(item.source) === "event_post";
          const published = item.is_published !== false;
          const url = mediaUrl(item);
          const title = stringValue(item.title || "Untitled media");
          const itemId = stringValue(item.id);
          const updating = operation === `toggle:${itemId}`;
          const visibilityLabel = published ? "Displayed on website" : "Hidden from website";
          return <article key={itemId} className={published ? "is-visible" : "is-hidden"}>
            <div className="media-library-cover">
              {video ? <video src={url} controls preload="metadata" /> : <img src={url} alt={stringValue(item.alt_text) || title} loading="lazy" decoding="async" />}
              <span className={`media-visibility-badge ${published ? "visible" : "hidden"}`}>{published ? <Eye size={12} /> : <EyeOff size={12} />}{visibilityLabel}</span>
              <i className="media-source-badge">{eventPost ? "Event post" : video ? "Video" : "Image"}</i>
            </div>
            <div className="media-library-copy">
              <b>{title}</b>
              <small>{eventPost ? "Website control only; the app School Gallery is unchanged." : stringValue(item.caption) || "No caption"}</small>
              <div className="media-card-actions">
                <span className={`media-state-pill ${published ? "visible" : "hidden"}`}>{published ? <CheckCircle2 size={13} /> : <EyeOff size={13} />}{visibilityLabel}</span>
                <button className="media-toggle" type="button" disabled={updating} aria-busy={updating} aria-pressed={published} aria-label={`${published ? "Hide" : "Display"} ${title} ${published ? "from" : "on"} the public website`} title={published ? "Hide from public website" : "Display on public website"} onClick={() => void toggle(item)}>{updating ? <LoadingIndicator label="Updating…" compact announce={false} /> : published ? "Hide" : "Display"}</button>
                <button type="button" aria-label={`Preview ${title}`} title="Preview media" onClick={() => setPreview(item)}><Eye size={15} /></button>
                {!eventPost && <><button type="button" aria-label={`Edit ${title}`} title="Edit media details" onClick={() => setEditing(item)}><Pencil size={15} /></button><button type="button" className="danger" aria-label={operation === `delete:${itemId}` ? `Deleting ${title}` : `Delete ${title}`} title="Delete media" disabled={operation === `delete:${itemId}`} onClick={() => void deleteMedia(item)}>{operation === `delete:${itemId}` ? <span className="activity-spinner" aria-hidden="true" /> : <Trash2 size={15} />}</button></>}
              </div>
            </div>
          </article>;
        })}</div> : <p className="ops-empty">No media matches the current search and filter.</p>}
      </section>
