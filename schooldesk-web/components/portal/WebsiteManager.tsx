...
export function WebsiteManager({ onNotify }: { onNotify: (message: string) => void }) {
  ...
  return <section className="ops-module gallery-workspace">
    ...
    {loading && tab !== "ticker" ? <PortalModuleSkeleton variant="split" label="Loading website and media settings" /> : tab === "gallery" ? <div className="media-library-layout">
      ...
      <section className="surface media-library-panel"><div className="media-library-toolbar"><div><h3>Public media library</h3><small>{visibleGallery.length} item{visibleGallery.length === 1 ? "" : "s"} shown · Event posts appear only when they target School Gallery</small></div><label className="media-search"><span className="sr-only">Search media</span><input value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Search media" /></label></div><div className="media-filter-row" aria-label="Filter media">{(["all", "image", "video", "published", "draft"] as MediaFilter[]).map((item) => <button key={item} type="button" className={filter === item ? "active" : ""} onClick={() => setFilter(item)}>{item === "all" ? "All" : item[0].toUpperCase() + item.slice(1)}</button>)}</div>{visibleGallery.length ? <div className="media-library-grid">{visibleGallery.map((item) => { const video = isVideo(item); const eventPost = stringValue(item.source) === "event_post"; const published = item.is_published !== false; const url = mediaUrl(item); const title = stringValue(item.title || "Untitled media"); const itemId = stringValue(item.id); return <article key={itemId}><div className="media-library-cover">{video ? <video src={url} controls preload="metadata" /> : <img src={url} alt={stringValue(item.alt_text) || title} loading="lazy" decoding="async" />}<span className={published ? "published" : "draft"}>{published ? "Published" : "Draft"}</span><i>{eventPost ? "Event post · School Gallery" : video ? "Video" : "Image"}</i></div><div className="media-library-copy"><b>{title}</b><small>{eventPost ? "Selected by the event post's School Gallery destination." : stringValue(item.caption) || "No caption"}</small><div>{eventPost ? <span className="status-pill">Managed in Event Posts</span> : <><button type="button" disabled={operation === `toggle:${itemId}`} aria-busy={operation === `toggle:${itemId}`} onClick={() => void toggle(item)}>{operation === `toggle:${itemId}` ? <LoadingIndicator label="Updating…" compact announce={false} /> : published ? "Unpublish" : "Publish"}</button><button type="button" aria-label={`Preview ${title}`} onClick={() => setPreview(item)}><Eye size={15} /></button><button type="button" aria-label={`Edit ${title}`} onClick={() => setEditing(item)}><Pencil size={15} /></button><button type="button" className="danger" aria-label={operation === `delete:${itemId}` ? `Deleting ${title}` : `Delete ${title}`} disabled={operation === `delete:${itemId}`} onClick={() => void deleteMedia(item)}>{operation === `delete:${itemId}` ? <span className="activity-spinner" aria-hidden="true" /> : <Trash2 size={15} />}</button></>}</div></div></article>; })}</div> : <p className="ops-empty">No media matches the current search and filter.</p>}</section>
