...
export function ReportsWorkspace({ role, onNotify }: {
  ...
  return <section className="ops-module leadership-reports">
    ...
    <div className="leadership-report-grid">
      {reports.map((report) => <article className={isFinanceReport(report) ? "finance" : "academic"} key={report.id}><span><FileText size={18} /></span><div><h3>{report.title}</h3><p>{report.description}</p></div><button type="button" className="secondary-button" onClick={() => void generate(report)} disabled={Boolean(exporting)}>{exporting === report.id ? "Preparing…" : <><Download size={15} /> Generate PDF</>}</button></article>)}
    </div>
