...
export function PreschoolEssentials({ site }: { site: PublicWebsite }) {
  ...
  const galleryItems = uniqueGalleryItems(gallery).slice(0, 6);
  useEffect(() => {
  ...
  return <>
    ...
    <section id="gallery" className="public-section gallery-section"><h2>Moments That Matter <i>❧</i></h2><div className="preschool-gallery">{galleryItems.length ? galleryItems.map((item) => <img key={`${item.id}-${item.media_url}`} src={item.media_url} alt={item.alt_text || item.title || "School gallery moment"} loading="lazy" decoding="async"/>) : <p className="empty-gallery">No school moments have been selected for the public gallery yet.</p>}</div></section>
