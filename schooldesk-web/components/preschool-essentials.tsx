...
const fallbackPrograms = [
  ["Daycare", "1.5 – 2 Years", "A caring, secure day where children feel at home while they learn and play.", "/preschool/programs/daycare.png"],
  ["Playgroup", "2 – 3 Years", "A joyful first school experience filled with sensory play, music, and connection.", "/preschool/programs/playgroup.png"],
  ["Nursery", "3 – 4 Years", "Stories, movement, friendship, and child-led exploration build growing confidence.", "/preschool/programs/nursery.png"],
  ["PP1", "4 – 5 Years", "Playful language and early numeracy experiences nurture independent thinkers.", "/preschool/programs/pp1.png"],
  ["PP2", "5 – 6 Years", "Meaningful discovery and school-readiness experiences prepare confident learners.", "/preschool/programs/pp2.png"],
];
...
export function PreschoolEssentials({ site }: { site: PublicWebsite }) {
  ...
  const gallery = site.gallery?.filter(isPublicGalleryImage).slice(0, 6) ?? [];
  ...
  const galleryItems = gallery.length ? gallery.map((item: GalleryItem) => ({ src: item.media_url, alt: item.alt_text || item.title })) : heroSlides.concat(heroSlides).map((item) => ({ src: item.image, alt: item.alt }));
  useEffect(() => {
  ...
  return <>
    ...
    <section id="gallery" className="public-section gallery-section"><h2>Moments That Matter <i>❧</i></h2><div className="preschool-gallery">{galleryItems.map((item, index) => <img key={`${item.src}-${index}`} src={item.src} alt={item.alt}/>)}</div></section>
