...
func RegisterV1Routes(r *gin.Engine, cfg *config.Config) {
	...
	api := r.Group("/api/v1")
	{
		...
		fees.Use(middleware.AuthMiddleware(), middleware.SchoolScopeMiddleware())
		{
			...
			fees.POST("/structures/rollover", middleware.RBACMiddleware("Principal"), middleware.RateLimitMiddleware("fee_write", cfg.RateLimitMaxAPI, time.Duration(cfg.RateLimitWindowSeconds)*time.Second), feeHandler.RolloverFeeStructures)
			...
			fees.POST("/payments/intent", middleware.RBACMiddleware("Parent"), middleware.RateLimitMiddleware("fee_write", cfg.RateLimitMaxAPI, time.Duration(cfg.RateLimitWindowSeconds)*time.Second), feeHandler.CreateFeePaymentIntent)
			...
			fees.PATCH("/payments/:id/resubmit", middleware.RBACMiddleware("Parent"), middleware.RateLimitMiddleware("fee_write", cfg.RateLimitMaxAPI, time.Duration(cfg.RateLimitWindowSeconds)*time.Second), feeHandler.ResubmitFeePayment)
			...
			fees.GET("/payment-configs", middleware.RBACMiddleware("Principal"), feeHandler.GetScopedPaymentConfigs)
			fees.POST("/payment-configs", middleware.RBACMiddleware("Principal"), middleware.RateLimitMiddleware("fee_write", cfg.RateLimitMaxAPI, time.Duration(cfg.RateLimitWindowSeconds)*time.Second), feeHandler.CreateScopedPaymentConfig)
			fees.PUT("/payment-configs/:id", middleware.RBACMiddleware("Principal"), middleware.RateLimitMiddleware("fee_write", cfg.RateLimitMaxAPI, time.Duration(cfg.RateLimitWindowSeconds)*time.Second), feeHandler.UpdateScopedPaymentConfig)
			fees.PATCH("/payment-configs/:id", middleware.RBACMiddleware("Principal"), middleware.RateLimitMiddleware("fee_write", cfg.RateLimitMaxAPI, time.Duration(cfg.RateLimitWindowSeconds)*time.Second), feeHandler.UpdateScopedPaymentConfig)
			fees.POST("/payment-configs/:id/qr", middleware.RBACMiddleware("Principal"), middleware.RateLimitMiddleware("fee_write", cfg.RateLimitMaxAPI, time.Duration(cfg.RateLimitWindowSeconds)*time.Second), feeHandler.UploadScopedPaymentQR)
			fees.GET("/concessions", middleware.RBACMiddleware("Principal", "Parent"), feeHandler.GetConcessions)
			fees.POST("/concessions", middleware.RBACMiddleware("Principal", "Parent"), middleware.RateLimitMiddleware("fee_write", cfg.RateLimitMaxAPI, time.Duration(cfg.RateLimitWindowSeconds)*time.Second), feeHandler.CreateConcession)
