...
class _ParentTeacherChatScreenState extends State<ParentTeacherChatScreen> {
  ...
  List<_TeacherThread> _mergeThreads({
    ...
    final byTeacher = <String, _TeacherThread>{
      for (final row in teacherRows) row.teacherId: row,
    };
    for (final row in conversations) {
      ...
      final existing = byTeacher[teacherId];
      byTeacher[teacherId] =
          (existing ??
                  _TeacherThread(
                    teacherId: teacherId,
  ...
  _TeacherThread? _selectThread(List<_TeacherThread> threads) {
    ...
    final selectedId = _selectedThread?.teacherId ?? '';
    return threads.firstWhere(
      (thread) => thread.teacherId == selectedId,
        ...
        Expanded(
          child: LayoutBuilder(
            builder: (context, constraints) {
              ...
              final chat = _chatPane(canSend: true);
              if (wide) {
                return Row(
                  children: [
  ...
  Widget _conversationList() {
    ...
    return ListView.separated(
      ...
      itemBuilder: (context, index) {
        ...
        final selected = thread.teacherId == _selectedThread?.teacherId;
        return ListTile(
          selected: selected,
  ...
  Widget _chatPane({required bool canSend}) {
    final thread = _selectedThread;
    if (thread == null) {
    ...
    return Column(
      children: [
        ListTile(
          ...
          subtitle: Text('${thread.studentName} - ${thread.subtitle}'),
        ),
        Expanded(
