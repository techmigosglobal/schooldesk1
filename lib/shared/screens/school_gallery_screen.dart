...
class SchoolGalleryScreen extends ConsumerStatefulWidget {
  const SchoolGalleryScreen({super.key});

  @override
  ConsumerState<SchoolGalleryScreen> createState() =>
      _SchoolGalleryScreenState();
}

class _SchoolGalleryScreenState extends ConsumerState<SchoolGalleryScreen> {
  ...
  bool get _canManagePosts => ref.watch(currentRoleNameProvider) == 'principal';

  String get _role => ref.watch(currentRoleNameProvider) ?? '';

  @override
  void initState() {
    super.initState();
    final repository = ref.read(eventPostRepositoryProvider);
    _stateController = RepositoryStateController<List<Map<String, dynamic>>>(
      reader: ({forceRefresh = false}) => guardApi(
        () => repository.loadGalleryPosts(forceRefresh: forceRefresh),
      ),
  ...
  Widget _buildBody(BuildContext context) {
    return SchoolDeskRepositoryStateView<List<Map<String, dynamic>>>(
      state: _state,
      onRetry: _loadGallery,
      loadingMessage: 'Loading gallery',
      errorTitle: 'Gallery unavailable',
      emptyTitle: 'No gallery posts yet',
      emptyMessage: 'Approved school gallery posts will appear here.',
      data: (posts) {
        if (posts.isEmpty) {
          final role = _role;
          ...
          return SchoolDeskStatusPanel.empty(
            ...
            onAction: actionRoute != null
                ? () => SchoolDeskNavigation.push(context, actionRoute)
                : null,
          );
        }
