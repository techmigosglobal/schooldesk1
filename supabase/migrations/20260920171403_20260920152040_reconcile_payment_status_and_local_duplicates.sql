-- Bring the hosted fee-payment RPC back to the repository's legacy-status
-- contract, validate the already-clean local status check, and remove local
-- duplicate indexes/read policies that production does not need.

alter table public.parent_payment_requests
  validate constraint parent_payment_requests_status_check;

-- The created_at index provides the same lookup as this local-only index.
drop index if exists public.idx_notification_events_retention;

-- These local read policies repeat the exact predicate of their existing
-- owner/school-scoped ALL policies. Removing them does not widen or narrow
-- access, and keeps fresh local databases aligned with production policy names.
drop policy if exists "Users can see own notification preferences"
  on public.notification_preferences;
drop policy if exists "Users can see own notification subscriptions"
  on public.notification_subscriptions;
drop policy if exists school_calendar_preferences_school_select
  on public.school_calendar_preferences;
drop policy if exists website_content_principal_read
  on public.school_website_content;
drop policy if exists website_gallery_principal_read
  on public.school_website_gallery_items;

-- Preserve legacy parent payment requests that still use the `pending` alias.
-- This is the same function body as the checked-in compatibility migration;
-- production currently differs only by omitting `pending` from the status guard.
